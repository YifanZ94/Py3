...
def measure():
    ...
    return [acc_x+0.39, acc_y-0.18, acc_z+0.6, gyro_x+0.19, gyro_y-2.06, gyro_z+2.41]  # bias
...
ave = np.zeros((6,6)) 
for i in range(6):
    ave[i,:] = np.mean(s1_array[i*10:(i+1)*10,], axis=0)

#%%
filename = input('enter the file name: ')       
file = open(filename + '_raw.txt', 'w')
np.savetxt(file, np.array(s1_array))
file.close()

file = open(filename + '_ave.txt', 'w')
np.savetxt(file, ave)
file.close()
