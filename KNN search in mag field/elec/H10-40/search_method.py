...
B = np.load('H10-40_B.npy')
LocMap = np.load('H10-40_LocMap.npy')
NormP = np.load('H10-40_NormP.npy')
...
import time
test = np.load('test_mag.npy')
ref = np.load('test_loc.npy')
location = []
...
plt.plot(0.5*x,npData[:,0], label="brute force")
plt.plot(0.5*x,ref[:,0], label="scikit learn")
plt.legend()
plt.show() 
print(end-start)   